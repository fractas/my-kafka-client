...
public sealed class MyHeaderCollection : IEnumerable<KeyValuePair<string, byte[]>>
{
  private readonly IDictionary<string, byte[]> _headers;

  public MyHeaderCollection()
    : this(new Dictionary<string, byte[]>())
  {
  }

  public MyHeaderCollection(IDictionary<string, byte[]> headers)
    : this(headers.ToArray())
  {
  }

  public MyHeaderCollection(IEnumerable<KeyValuePair<string, byte[]>> headers)
    : this(headers.ToArray())
  {
  }
  ...
  {
    return _headers.TryGetValue(key, out byte[]? value) ? value : Array.Empty<byte>();
  }
